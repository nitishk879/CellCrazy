...
@section('content')
    <form class="" action="{{ route('admin.brands.update', $brand->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="form-group">
                <label for="BrandName">Brand Name</label>
                <input type="text" name="name" class="form-control form-control-sm @error('name') is-invalid @enderror" id="BrandName" placeholder="Brand Name" value="{{  $brand->name ?? old('name') ?? '' }}">
            </div>
            <div class="form-group">
                @error('name')
